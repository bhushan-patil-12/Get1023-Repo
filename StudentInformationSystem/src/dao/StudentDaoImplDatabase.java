...
public class StudentDaoImplDatabase implements StudentDao {
	...
	public Student findStudentByRollNo(int rollNo) {
		String sql = "select * from tbl_students where roll_no=?";
	...
	public List<Student> viewAllStudent() {
		...
		try {
			ps = conn.prepareStatement(sql);
			rs = ps.executeQuery();
			// 1. fetch all row data 2. wrap in student object 3. add student object in collections
	...
	public String registrationDb(Registration registration) {
		...
		try {
			...
			int count = ps.executeUpdate();
			message = count>0? "Registration successfull": "Error";
